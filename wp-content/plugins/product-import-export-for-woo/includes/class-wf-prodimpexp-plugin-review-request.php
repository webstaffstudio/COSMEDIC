...
class Product_import_export_Review_Request
{
    ...
    private $plugin_prefix              =   "wfpipe"; /* must be unique name */
    private $activation_hook            =   "wfpipe_activate"; /* hook for activation, to store activated date */
    private $deactivation_hook          =   "wfpipe_deactivate"; /* hook for deactivation, to delete activated date */
    ...
    public function __construct()
    {
        ...
        if ($this->check_condition()) /* checks the banner is active now */ {
            $this->banner_message = sprintf(__("Hey, we at %sWebToffee%s would like to thank you for using our plugin. We would really appreciate if you could take a moment to drop a quick review that will inspire us to keep going.", 'product-import-export-for-woo'), '<b>', '</b>');
...
?>
        <div class="<?php echo $this->banner_css_class; ?> notice-info notice is-dismissible">
            ...
            <div class="wt-cli-review-footer" style="position: relative;">
                <span class="wt-cli-footer-icon" style="position: absolute;right: 0;bottom: 10px;"><img src="<?php echo plugins_url(basename(plugin_dir_path(WF_ProdImpExpCsv_FILE))).$this->webtoffee_logo_url; ?>" style="max-width:100px;"></span>
            </div>
        </div>
    <?php
    }
