			...
			<?php if ($slider && is_array($slider)): ?>
				<div class="hero-slider">
					<?php foreach ($slider as $slide): ?>
						<a class="hero-slider__content"
						   href="<?= (isset($slide['link']['url']) && $slide['link']['url']) ? $slide['link']['url'] : '#' ?>"
						   <?= (isset($slide['link']['target']) && $slide['link']['target']) ? 'target="_blank"' : 'target="_self"'; ?>
						   style="background-image: url(<?php echo wp_get_attachment_image_url($slide['image']['id'], 'full'); ?>);">
							<?php if ($slide['title']): ?>
				...
				<ul class="store-grid">
					<li>
						<a href="<?php echo esc_url(home_url('/store')); ?>">
							<img
								src="<?php echo wp_get_attachment_image_src($fields['image_for_all_product'], 'full')[0]; ?>"
								alt="">
